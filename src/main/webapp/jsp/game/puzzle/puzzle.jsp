...
<script>
    ...
    function numbering() {
        // 결과를 표시할 element
        const numberingElement = document.getElementById('numbering');

        // 현재 화면에 표시된 값
        let number = parseInt(numberingElement.innerText);

        // 더하기 전에 숫자가 maxCount를 초과하지 않도록 제한
        if (number < maxCount) {
            number = number + 1;
            numberingElement.innerText = number + "/5";
        }
    ...
    function scoring() {
        ...
        // 현재 화면에 표시된 값
        score = parseInt(scoringElement.innerText);

        // 더하기 전에 숫자가 maxCount를 초과하지 않도록 제한
        if (score < maxCount) {
            score = score + 1;
            scoringElement.innerText = score;
        }
    }
    ...
    function updateScore() {
        // 화면에 점수 표시
        const scoringElement = document.getElementById('scoring');
        scoringElement.innerText = score;  // 실시간으로 점수 반영
    }

    // 게임 종료 시 점수 및 메시지 표시
    function showGameOver() {
        const gameOverMessage = document.getElementById('game-over-message');
        gameOverMessage.style.display = 'block'; // "게임이 끝났습니다!" 메시지 보이기

        // 최종 점수를 화면에 표시 (뒤에 "!!!"를 붙여서 표시)
        const finalScoreElement = document.getElementById('scoring');
        finalScoreElement.innerText = score + `개 정답!`;  // 점수 뒤에 "개 정답!"을 추가
    }
    ...
    function restartGame() {
        ...
        // numbering 숫자 초기화 (1로 설정)
        document.getElementById('numbering').innerText = '1/5'; // 게임 시작 시 numbering 1/5로 초기화

        // 최종 점수 초기화 (0으로 설정)
        document.getElementById('scoring').innerText = '0';  // 게임 시작 시 score 0으로 초기화
    }

    // 다음 문제로 넘어가기
    function nextQuestion() {
        getRandomQuestion(); // 문제 가져오기
        numbering(); // 문제 numbering 증가
    }
